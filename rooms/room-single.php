...
if (isset($_GET['id'])) {
	...
	if (isset($_POST['submit'])) {
		if (empty($_POST['email']) or empty($_POST['phone_number']) or empty($_POST['full_name']) or empty($_POST['check_in']) or empty($_POST['check_out'])) {
			...
			if (new DateTime(date("Y/m/d")) > $check_in or new DateTime(date("Y/m/d")) > $check_out) {
				echo "<script>alert(`pick a date that is not in the past`)</script>";
			} else {
				if ($check_in > $check_out or $check_in == new DateTime(date("Y/m/d"))) {
					echo "<script>alert(`pick a date that is not today for check_in and pick a check-in date that less that check-out date `)</script>";
				} else {
